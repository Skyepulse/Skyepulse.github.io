        ...
        <!-- content -->
         <div class="flex flex-col items-start justify-center w-[70%] ">
            ...
            <img :src="image" class="w-full h-auto rounded-lg border-2 border-black mt-2 mb-2" alt="Project Image" 
                :class="{'cursor-pointer hover:scale-102 transition-all duration-200': !isMobile}"
                @click="handleClick"
            />
            ...
            <div class="flex flex-row items-center justify-start w-full pl-2 mb-4">
                <template v-if="link.startsWith('http://') || link.startsWith('https://')">
                    <a :href="link" target="_blank" class="text-2xl text-gray-600 underline font-bold hover:underline">Visit Website</a>
                </template>
                <template v-else>
                    <a :href="link" target="_blank" class="text-2xl text-gray-600 underline font-bold hover:underline">Learn More</a>
                </template>
            </div>
         </div>
    </div>
